...
ALERT_NET_USD = float(os.environ.get("ALERT_NET_USD", 80_000))  # 60 秒內淨力道門檻
...
def fetch_binance_universe():
    ...
    return {t["symbol"] for t in tick if t["symbol"] in eligible and float(t["quoteVolume"]) >= MIN_QUOTE_VOL}
...
def send_discord_alert(symbol, exch, net, window_sec):
    if not DISCORD_WEBHOOK:
        log.info(f"[DRY] would alert {symbol} net={net:+.0f} (no webhook set)")
        return
    direction = "買方主動" if net > 0 else "賣方主動"
    title = f"⚡ 即時警報．{symbol}．{window_sec}秒內{direction}湧入"
    desc = (f"近 {window_sec} 秒大額成交淨力道 ${net:+,.0f}\n"
            f"交易所：{exch}\n"
...
async def checker_loop():
    while True:
        ...
        for symbol in list(trades.keys()):
            ...
            if abs(net) < ALERT_NET_USD:
                continue
            if now - last_alert.get(symbol, 0) < COOLDOWN_SEC:
                continue
            exch = "binance" if symbol in binance_symbols else "gate"
            log.info(f"ALERT {symbol} net={net:+.0f} exch={exch}")
            send_discord_alert(symbol, exch, net, WINDOW_SEC)
...
async def main():
    log.info(f"啟動：門檻=${ALERT_NET_USD:,.0f}/{WINDOW_SEC}s，冷卻={COOLDOWN_SEC}s，"
